...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        if diff<=0:
            newsentence=sentence[0:max_length]
            newlabels=labels[0:max_length]
            mask=[1]*max_length
        else:
            newsentence=sentence+[zero_vector]*diff
            newlabels=labels+[zero_label]*diff
            mask=[1]*len(sentence)+[0]*diff
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings=tf.reshape(tf.nn.embedding_lookup(my_embeddings,self.input_placeholder),[-1,self.max_length,self.config.n_features*self.config.embed_size])
        ### END YOUR CODE
        return embeddings
