...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        newsentence=[start]*window_size+sentence+[end]*window_size
        for i in range(len(labels)):
            s=[]
            for j in range(2*window_size+1):
                s=s+newsentence[i+j]
            l=labels[i]
            windowed_data.append((s,l))
    ### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (!3-5 lines)
        my_embeddings=tf.Variable(self.pretrained_embeddings)
        embeddings=tf.reshape(tf.nn.embedding_lookup(my_embeddings,self.input_placeholder),[-1,self.config.n_window_features*self.config.embed_size])
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope("ner"):
            ...
            b1=tf.get_variable("b1",initializer=tf.zeros([self.config.hidden_size,],tf.float32))
            ...
            b2=tf.get_variable("b2",initializer=tf.zeros([self.config.n_classes,],tf.float32))
